...
export function KanbanAprovacoes({
  ...
  const colunas = useRef<Partial<Record<StatusAprovacao, HTMLDivElement>>>({});
  const arrastou = useRef(false);
  const [alvo, setAlvo] = useState<StatusAprovacao | null>(null);
  const reduzMovimento = useReducedMotion();

  function colunaEmbaixo(x: number, y: number): StatusAprovacao | null {
  ...
  function mover(a: Aprovacao, direcao: -1 | 1) {
    const i = ETAPAS.findIndex((e) => e.id === a.status);
    const proxima = ETAPAS[i + direcao];
    if (proxima) onAtualizar(a.id, { status: proxima.id });
  }
              ...
              {daEtapa.map((a) => (
                <motion.article
                  ...
                  onDragEnd={(e) => {
                    ...
                    if (destino && destino !== a.status) {
                      onAtualizar(a.id, { status: destino });
                    }
                    setAlvo(null);
                ...
                >
                  <button
                    type="button"
                    onClick={() => {
                      if (!arrastou.current) onAbrir(a.email);
                    }}
                    className="block w-full text-left"
                  >
                    <p className="text-body truncate font-semibold">{a.nome}</p>
